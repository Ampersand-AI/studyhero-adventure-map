...
interface Curriculum {
  id?: string; // Make id optional to support both patterns
  subject: string;
...
class ClaudeService {
  ...
  constructor() {
    ...
    this.apiKey = 'dummy-key-for-demonstration';
  ...
  async getSubjectTopics(subject: string, className: string) {
    try {
      ...
      // Check if anthropic was initialized successfully
      if (!this.anthropic) {
  ...
  async getLessonContent(subject: string, topic: string, className: string = '10') {
    try {
      ...
      if (!this.anthropic) {
        throw new Error('Anthropic SDK not initialized properly');
        // Instead of throwing, use fallback to allow development without API key
      }
